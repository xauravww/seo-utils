...
for (const cat of categories) {
  ...
  queueEvents.on("completed", async ({ jobId, returnvalue }) => {
    console.log(
      `[DEBUG] [${cat}] Job ${jobId} completed event triggered with returnvalue:`,
      returnvalue,
    );
    const jobData = await getJobData(jobId);
    ...
    console.log(
      `[${requestId}] [${cat}] Job ${jobId} completed for campaign ${campaign_id}`,
    );

    // REDIS FLAG CHECK: Check if publishWorker.js already updated database
    console.log(
      `[${requestId}] 🔍 COMPLETED HANDLER DEBUG: Starting Redis flag check for campaign ${campaign_id}`,
    );
    console.log(
      `[${requestId}] 📊 COMPLETED HANDLER DEBUG: Job details - ID: ${jobId}, Category: ${cat}, Return value: ${JSON.stringify(returnvalue)}`,
    );

    if (campaign_id) {
      try {
        console.log(
          `[${requestId}] 🚩 CHECKING Redis flag: campaign_db_updated:${campaign_id}`,
        );
        const dbUpdatedFlag = await redis.get(
          `campaign_db_updated:${campaign_id}`,
        );
        console.log(
          `[${requestId}] 🚩 FLAG RESULT: ${dbUpdatedFlag} (type: ${typeof dbUpdatedFlag})`,
        );

        if (dbUpdatedFlag === "true") {
          console.log(
            `[${requestId}] 🛡️ WORKER PROTECTION ACTIVATED: Database already updated by publishWorker.js for campaign ${campaign_id}`,
          );
          console.log(
            `[${requestId}] 🚫 COMPLETED HANDLER EXITING IMMEDIATELY to prevent database override`,
          );
          console.log(
            `[${requestId}] 🧹 Cleaning up Redis flag: campaign_db_updated:${campaign_id}`,
          );
          ...
          websocketLogger.log(
            requestId,
            `✅ WORKER PROTECTION: Campaign ${campaign_id} database already updated by worker. Completed handler skipped.`,
            "info",
          );
          return; // Exit immediately
        } else {
          console.log(
            `[${requestId}] 📝 NO FLAG FOUND: Completed handler will continue with normal processing`,
          );
          console.log(
            `[${requestId}] 🔄 This indicates either: 1) Normal campaign, or 2) Worker hasn't set flag yet`,
          );
        }
      } catch (flagCheckError) {
        console.error(
          `[${requestId}] ❌ FLAG CHECK ERROR: ${flagCheckError.message}`,
        );
        console.error(
          `[${requestId}] 🔄 Continuing with normal processing despite flag check error`,
        );
      }
    } else {
      console.log(
        `[${requestId}] ⚠️ NO CAMPAIGN ID: Skipping flag check (campaign_id is ${campaign_id})`,
      );
    }
    ...
    if (campaign_id) {
      ...
      if (message.status === "done") {
        ...
        console.log(
          `[${requestId}] [${cat}] Job ${jobId} marked as successful`,
        );
      }
      const remaining = await redis.scard(`campaign_jobs:${campaign_id}`);
      console.log(
        `[${requestId}] Campaign ${campaign_id}: ${remaining} jobs remaining`,
      );
      if (remaining === 0) {
        // AGGRESSIVE RETRY DETECTION: Check database first before any processing
        console.log(
          `[${requestId}] AGGRESSIVE RETRY CHECK: Checking database before processing campaign ${campaign_id}`,
        );
        ...
        try {
          ...
          if (
            quickCheckResponse.data &&
            quickCheckResponse.data.logs &&
            Object.keys(quickCheckResponse.data.logs).length > 0
          ) {
            console.log(
              `[${requestId}] AGGRESSIVE RETRY DETECTION: Campaign ${campaign_id} already has logs in database. SKIPPING all processing to prevent override.`,
            );
            ...
            websocketLogger.log(
              requestId,
              `✅ RETRY PREVENTED: Campaign ${campaign_id} already has logs. Skipped processing to prevent override.`,
              "info",
            );
            return; // Exit immediately
          }
          ...
          console.log(
            `[${requestId}] Quick retry check failed: ${quickCheckError.message}`,
          );
        }

        // All jobs for this campaign are done, get aggregated logs
        console.log(
          `[${requestId}] Starting log aggregation for campaign ${campaign_id}`,
        );
        const campaignLogsData = await redis.get(
        ...
        if (campaignLogsData) {
          try {
            ...
            console.log(
              `[${requestId}] Found ${Object.keys(aggregatedLogs).length} log categories for campaign ${campaign_id}`,
            );
          } catch (parseError) {
            console.error(
              `[${requestId}] Error parsing campaign logs for ${campaign_id}:`,
              parseError,
            );
            // Fallback: try old format in case of mixed data
            try {
              const logEntries = await redis.lrange(
                `campaign_logs:${campaign_id}`,
                0,
                -1,
              );
              console.log(
                `[${requestId}] Fallback: found ${logEntries.length} log entries in old format`,
              );
              for (const entry of logEntries) {
                const { logs } = JSON.parse(entry);
                for (const [cat, logObj] of Object.entries(logs)) {
                  if (!aggregatedLogs[cat])
                    aggregatedLogs[cat] = { logs: [], result: "" };
                  aggregatedLogs[cat].logs.push(...(logObj.logs || []));
                  aggregatedLogs[cat].result =
                    logObj.result || aggregatedLogs[cat].result;
                }
              }
            } catch (fallbackError) {
              console.error(
                `[${requestId}] Fallback parsing also failed for ${campaign_id}:`,
                fallbackError,
              );
            }
          }
          ...
          console.log(
            `[${requestId}] No campaign logs found for ${campaign_id}`,
          );
        }
        // Compose result string
        const totalCount = parseInt(
          (await redis.get(`campaign_total:${campaign_id}`)) || "0",
          10,
        );
        const successCount = parseInt(
          (await redis.get(`campaign_success:${campaign_id}`)) || "0",
          10,
        );
        ...
        console.log(
          `[${requestId}] Campaign ${campaign_id} final stats: ${resultString}, status: ${finalStatus}`,
        );
        console.log(
          `[${requestId}] QUEUE EVENT DEBUG: Job completed, checking if should update database`,
        );
        console.log(
          `[${requestId}] QUEUE EVENT DEBUG: Total jobs for campaign: ${totalCount}, Success: ${successCount}`,
        );
        ...
        if (campaignLogsData) {
          try {
            ...
            if (parsedData.attempts || parsedData.results) {
              ...
              const hasRetries = Object.values(parsedData.attempts || {}).some(
                (attempts) => Array.isArray(attempts) && attempts.length > 1,
              );

              if (hasRetries) {
                isRetryScenario = true;
                console.log(
                  `[${requestId}] RETRY DETECTED: Found multiple attempts in Redis logs for campaign ${campaign_id}`,
                );
              }
            }
          } catch (parseError) {
            console.log(
              `[${requestId}] Could not parse Redis logs for retry detection: ${parseError.message}`,
            );
          }
        }
        ...
        if (!isRetryScenario && campaign_id && user_id) {
          try {
            const authToken = process.env.UTIL_TOKEN;
            const apiUpdateUrl = `${process.env.MAIN_BACKEND_URL}/api/v1/campaigns/${campaign_id}`;
            const existingCampaignResponse = await axios.get(
              `${apiUpdateUrl}`,
              {
                headers: {
                  accept: "application/json",
            ...
            if (existingCampaignResponse.data) {
              ...
              if (
                existingCampaignStatus === "completed" ||
                existingCampaignStatus === "failed"
              ) {
                isRetryScenario = true;
                console.log(
                  `[${requestId}] RETRY DETECTED: Campaign ${campaign_id} was already ${existingCampaignStatus}, processing ${totalCount} job(s). This appears to be a retry.`,
                );
              }
            }
          } catch (fetchError) {
            console.log(
              `[${requestId}] Could not fetch existing campaign status: ${fetchError.message}`,
            );
          }
        }
        ...
        if (isRetryScenario) {
          console.log(
            `[${requestId}] 🛡️ RETRY SCENARIO: Database already updated by publishWorker.js to prevent override`,
          );
          console.log(
            `[${requestId}] 🔄 Queue handler skipping database update for retry protection`,
          );

          // Clean up Redis keys and exit early
          console.log(
            `[${requestId}] Cleaning up Redis keys for retry scenario campaign ${campaign_id}`,
          );
          await redis.del(`campaign_jobs:${campaign_id}`);
          await redis.del(`campaign_total:${campaign_id}`);
          await redis.del(`campaign_success:${campaign_id}`);
          ...
          websocketLogger.log(
            requestId,
            `✅ Retry completed for campaign ${campaign_id}. Database updated by worker with proper log merging.`,
            "info",
          );
        ...
        console.log(
          `[${requestId}] 📝 NORMAL SCENARIO: Queue handler will update database as usual`,
        );
        ...
        if (campaign_id && user_id) {
          try {
            ...
            try {
              ...
              if (
                existingCampaignResponse.data &&
                existingCampaignResponse.data.logs
              ) {
                existingLogs = existingCampaignResponse.data.logs;
                console.log(
                  `[${requestId}] Found existing logs for campaign ${campaign_id}, merging with new logs`,
                );
              }
            } catch (fetchError) {
              console.log(
                `[${requestId}] Could not fetch existing campaign logs (${fetchError.message}), proceeding with new logs only`,
              );
            }
                ...
                if (existingLogs[category]) {
                  ...
                  try {
                    const existingCategoryData =
                      typeof existingLogs[category] === "string"
                    ...
                    const mergedCategoryData = {
                      ...
                      totalAttempts:
                        (existingCategoryData.totalAttempts || 1) + 1,
                      isRetry: true,
                    };

                    mergedLogs[category] = JSON.stringify(mergedCategoryData);
                    console.log(
                      `[${requestId}] MERGED logs for category ${category} (attempt #${mergedCategoryData.totalAttempts})`,
                    );
                  } catch (parseError) {
                    console.log(
                      `[${requestId}] Could not parse existing logs for ${category}, using new logs only`,
                    );
            ...
            websocketLogger.log(
              requestId,
              `✅ Campaign ${campaign_id} updated with categorized logs. API Response Status: ${apiResponse.status}, Result: ${resultString}`,
            );
            console.log(
              `[${requestId}] Campaign ${campaign_id} updated.`,
              apiResponse.data,
            );
          } catch (apiError) {
            websocketLogger.log(
              requestId,
              `❌ Failed to update campaign ${campaign_id} with logs: ${apiError.message}`,
              "error",
            );
            console.error(
              `[${requestId}] Error updating campaign ${campaign_id}:`,
              apiError.message,
            );
            if (apiError.response) {
              console.error(
                `[${requestId}] API Error Details:`,
          ...
          websocketLogger.log(
            requestId,
            `[Worker Update] Skipping campaign update: missing campaignId or userId.`,
            "warning",
          );
          console.log(
            `[${requestId}] Skipping campaign update. Missing campaignId or userId.`,
          );
        }
        // Clean up Redis keys
        console.log(
          `[${requestId}] Cleaned up Redis keys for campaign ${campaign_id}`,
        );
        await redis.del(`campaign_jobs:${campaign_id}`);
        await redis.del(`campaign_total:${campaign_id}`);
        await redis.del(`campaign_success:${campaign_id}`);
    ...
    if (requestId) {
      websocketLogger.log(
        requestId,
        `[BullMQ] Job ${jobId} completed: ${JSON.stringify(returnvalue)}`,
        "success",
      );
    }
  });
  ...
  queueEvents.on("failed", async ({ jobId, failedReason }) => {
    console.log(
      `[DEBUG] [${cat}] Job ${jobId} failed event triggered with reason:`,
      failedReason,
    );
    const jobData = await getJobData(jobId);
    const requestId = jobData?.requestId;
    ...
    console.log(
      `[${requestId}] [${cat}] Job ${jobId} failed for campaign ${campaign_id}`,
    );

    // REDIS FLAG CHECK: Check if publishWorker.js already updated database
    console.log(
      `[${requestId}] 🔍 FAILED HANDLER DEBUG: Starting Redis flag check for campaign ${campaign_id}`,
    );
    console.log(
      `[${requestId}] 📊 FAILED HANDLER DEBUG: Job details - ID: ${jobId}, Category: ${cat}, Failed reason: ${failedReason}`,
    );
    ...
    if (campaign_id) {
      try {
        ...
        if (dbUpdatedFlag === "true") {
          ...
          console.log(
            `[${requestId}] 🚫 FAILED HANDLER EXITING IMMEDIATELY to prevent database override`,
          );
          console.log(
            `[${requestId}] 🧹 Cleaning up Redis flag: campaign_db_updated:${campaign_id}`,
          );
          ...
          websocketLogger.log(
            requestId,
            `✅ WORKER PROTECTION: Campaign ${campaign_id} database already updated by worker. Failed handler skipped.`,
            "info",
          );
          return; // Exit immediately
        } else {
          console.log(
            `[${requestId}] 📝 NO FLAG FOUND: Failed handler will continue with normal processing`,
          );
          console.log(
            `[${requestId}] 🔄 This indicates either: 1) Normal campaign, or 2) Worker hasn't set flag yet`,
          );
    ...
    if (campaign_id) {
      ...
      if (remaining === 0) {
        ...
        console.log(
          `[${requestId}] QUEUE EVENT DEBUG: Job failed, checking if should update database`,
        );
        console.log(
          `[${requestId}] QUEUE EVENT DEBUG: Total jobs for campaign: ${totalCount}, Success: ${successCount}`,
        );
        ...
        if (campaign_id && user_id) {
          try {
            ...
            console.log(
              `[${requestId}] Campaign ${campaign_id} updated successfully.`,
              apiResponse.data,
            );
          } catch (apiError) {
            websocketLogger.log(
    ...
    if (requestId) {
      websocketLogger.log(
        requestId,
        `[BullMQ] Job ${jobId} failed: ${failedReason}`,
        "error",
      );
    }
  });
            ...
            <body>
              ...
              <table>
                ...
                ${Object.entries(queueStats)
                  .map(
                    ([cat, stats]) =>
                      `<tr><td>${cat}Queue</td><td>${stats.waiting}</td><td>${stats.active}</td><td>${stats.completed}</td><td>${stats.failed}</td></tr>`,
                  )
                  .join("")}
              </table>
              <table>
        ...
        <form id="removeForm">
          <label>Queue Name:
            <select name="queueName">
              ${Object.keys(queues)
                .map((q) => `<option value="${q}Queue">${q}Queue</option>`)
                .join("")}
            </select>
          </label>
...
server.listen(PORT, async () => {
  ...
  console.log(
    `BullMQ dashboard is available at http://localhost:${PORT}/admin/queues`,
  );
});
