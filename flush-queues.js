  ...
  for (const cat of categories) {
    ...
    for (const job of activeJobs) {
      try {
        ...
        allRemoved = false;
        console.error(
          `Failed to remove job ${job.id} from ${cat}Queue: ${err.message}`,
        );
      }
    }
    if (!allRemoved && activeJobs.length > 0) {
      ...
      try {
        ...
        console.log(
          `Obliterated all jobs in ${cat}Queue (including active jobs)`,
        );
      } catch (obliterateErr) {
        console.error(
          `Failed to obliterate ${cat}Queue: ${obliterateErr.message}`,
        );
      }
    }
  }
