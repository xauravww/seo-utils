...
class DiigoBookmarkingAdapter extends BaseAdapter {
  ...
  async determineCategory(contentData) {
    try {
      ...
      try {
        ...
        if (rawContent.startsWith("[") && rawContent.includes("]")) {
          ...
          if (numberedMatches && numberedMatches.length > 0) {
            ...
            const categoryMatches = rawContent.match(
              /(?:\*\s*|\-\s*|^\s*)([A-Za-z_]+)(?:\s*$|\s*\n)/gm,
            );
            if (categoryMatches) {
      ...
      this.log(
        `[EVENT] LLM determined categories: ${categories.join(", ")}`,
        "info",
        true,
      );
      ...
      this.log(
        `[ERROR] Failed to determine category: ${error.message}`,
        "error",
        true,
      );
  ...
  buildCategoryPrompt(contentData) {
    ...
    if (content.body || content.description) {
      const contentText = (content.body || content.description).substring(
        0,
        300,
      );
    ...
    prompt +=
      "Based on all this information about me, my business, and the content I'm posting, please determine the most appropriate categories for this content distribution.";
  ...
  getGroupUrls(categories) {
    ...
    categories.forEach((category) => {
      if (this.categoryGroups.hasOwnProperty(category)) {
        ...
        this.log(
          `[EVENT] Added groups for category "${category}": ${urls.join(", ")}`,
          "info",
          true,
        );
      } else {
        this.log(
          `[WARNING] No groups found for category "${category}" - skipping`,
          "warning",
          true,
        );
      }
  ...
  async publish() {
    this.log(
      `[EVENT] Entering DiigoBookmarkingAdapter publish method.`,
      "info",
      true,
    );
    let browser;
    ...
    try {
      ...
      this.log(
        "[EVENT] Determining appropriate category using LLM...",
        "detail",
        false,
      );
      ...
      this.log(
        "[DEBUG] Launching Chromium with CAPTCHA solver extension...",
        "detail",
        false,
      );
      this.log(
        `[DEBUG] Extension path: ${this.extensionPath}`,
        "detail",
        false,
      );
      ...
      this.log(
        `[EVENT] Navigating to login page: ${this.loginUrl}`,
        "detail",
        false,
      );
      ...
      this.log(
        "[EVENT] Filling login form with human-like behavior...",
        "detail",
        false,
      );
      ...
      const username =
        ...
        this.website.credentials?.email ||
        this.website.username ||
        this.website.email;
      const password =
        this.website.credentials?.password || this.website.password;

      if (!username || !password) {
        throw new Error(
          "Username/email and password are required for Diigo login",
        );
      }
      ...
      this.log(
        "[EVENT] Login form filled. Waiting for human-like delay before login...",
        "detail",
        false,
      );
      ...
      while (!loginSuccessful && loginAttempts < maxLoginAttempts) {
        loginAttempts++;
        this.log(
          `[EVENT] Login attempt ${loginAttempts}/${maxLoginAttempts}`,
          "detail",
          false,
        );
        ...
        try {
          ...
          if (isLoginButtonVisible) {
            this.log(
              "[EVENT] Login button still visible - attempting human-like click",
              "detail",
              false,
            );
            ...
            try {
              ...
              this.log(
                "[EVENT] Clicked login button with human-like behavior",
                "detail",
                false,
              );
            } catch (clickError) {
              this.log(
                "[DEBUG] Could not click login button, may be disabled",
                "detail",
                false,
              );
            }
          ...
          try {
            ...
            this.log(
              "[DEBUG] Still on login page, waiting for CAPTCHA completion",
              "detail",
              false,
            );
          ...
          try {
            await page.waitForSelector("div.addItemButton.blue", {
              state: "visible",
              timeout: 10000,
            });
            loginSuccessful = true;
          ...
          this.log(
            `[DEBUG] Login attempt ${loginAttempts} failed: ${error.message}`,
            "detail",
            false,
          );
          await page.waitForTimeout(5000);
      ...
      this.log(
        "[EVENT] Login successful after CAPTCHA completion.",
        "info",
        true,
      );
      ...
      this.log(
        `[EVENT] Available groups for categories [${categories.join(", ")}]: ${groupUrls.join(", ")}`,
        "info",
        true,
      );
      ...
      if (groupUrls.length === 0) {
        throw new Error(
          `No groups found for categories: ${categories.join(", ")}`,
        );
      }

      // Step 4: Randomly pick a group URL
      const randomGroupUrl =
        groupUrls[Math.floor(Math.random() * groupUrls.length)];
      this.log(
        `[EVENT] Randomly selected group: ${randomGroupUrl}`,
        "info",
        true,
      );
      ...
      if (groupNameMatch) {
        ...
        this.log(
          `[EVENT] Joining group directly via: ${joinUrl}`,
          "info",
          true,
        );
      ...
      this.log(
        `[EVENT] Navigating to group: ${randomGroupUrl}`,
        "detail",
        false,
      );
      ...
      try {
        ...
        const applyButton = page.locator(
          'a:has-text("Apply to join group"), button:has-text("Apply to join group")',
        );
        const isApplyButtonVisible = await applyButton.isVisible({
          timeout: 3000,
        });

        if (isApplyButtonVisible) {
          this.log(
            '[EVENT] Found "Apply to join group" - this group requires approval. Skipping to try another group...',
            "warning",
            true,
          );

          // Get a different group URL and try again
          const remainingGroups = groupUrls.filter(
            (url) => url !== randomGroupUrl,
          );
          if (remainingGroups.length > 0) {
            const newRandomGroupUrl =
              remainingGroups[
                Math.floor(Math.random() * remainingGroups.length)
              ];
            this.log(
              `[EVENT] Trying different group: ${newRandomGroupUrl}`,
              "info",
              true,
            );

            await page.goto(newRandomGroupUrl, {
              waitUntil: "domcontentloaded",
            });
            await page.waitForTimeout(5000);

            // Check this new group for apply button too
            const newApplyButton = page.locator(
              'a:has-text("Apply to join group"), button:has-text("Apply to join group")',
            );
            const isNewApplyButtonVisible = await newApplyButton.isVisible({
              timeout: 3000,
            });

            if (isNewApplyButtonVisible) {
              this.log(
                "[EVENT] Second group also requires approval. Proceeding anyway...",
                "warning",
                true,
              );
            }
        ...
        const joinButton = page.locator(
          'a.inviteBtn:has-text("Join this group")',
        );
        ...
        if (isJoinButtonVisible) {
          ...
          if (groupNameMatch) {
            ...
            this.log(
              `[EVENT] Found "Join this group" button - joining directly via: ${joinUrl}`,
              "info",
              true,
            );
            await page.goto(joinUrl, { waitUntil: "domcontentloaded" });
            ...
            this.log(
              "[EVENT] Successfully joined group and returned to group page",
              "info",
              true,
            );
          } else {
            this.log(
              "[EVENT] Could not extract group name from URL, proceeding without joining",
              "warning",
              true,
            );
          }
          ...
          this.log(
            '[EVENT] No "Join this group" button found - we are likely already a member',
            "info",
            true,
          );
        }
        ...
        this.log(
          "[EVENT] Could not find join button - we are likely already a member of this group",
          "info",
          true,
        );
      }
      ...
      this.log(
        "[EVENT] Now part of the group, creating bookmark post...",
        "info",
        true,
      );
      ...
      try {
        ...
        for (const selector of possibleSelectors) {
          try {
            ...
            this.log(
              `[EVENT] Found Bookmark link with selector: ${selector}`,
              "info",
              true,
            );
            break;
            ...
            this.log(
              `[DEBUG] Selector ${selector} not found, trying next...`,
              "detail",
              false,
            );
            continue;
        ...
        if (!bookmarkLink || !foundSelector) {
          ...
          const debugCloudinaryResult =
            await cloudinary.uploader.upload(debugScreenshotPath);
          fs.unlinkSync(debugScreenshotPath);
          this.log(
            `[DEBUG] Debug screenshot uploaded: ${debugCloudinaryResult.secure_url}`,
            "info",
            true,
          );

          throw new Error(
            "Could not find Bookmark link with any of the attempted selectors",
          );
        }
        ...
        while (!urlInput && bookmarkClickAttempts < maxBookmarkClickAttempts) {
          ...
          this.log(
            `[EVENT] Clicking Bookmark link (attempt ${bookmarkClickAttempts}/${maxBookmarkClickAttempts})...`,
            "info",
            true,
          );
          ...
          for (const selector of possibleUrlSelectors) {
            try {
              ...
              this.log(
                `[EVENT] Found URL input with selector: ${selector}`,
                "info",
                true,
              );
              break;
              ...
              this.log(
                `[DEBUG] URL selector ${selector} not found, trying next...`,
                "detail",
                false,
              );
              continue;
          ...
          if (urlInput) {
            this.log(
              `[EVENT] URL field found after ${bookmarkClickAttempts} bookmark click attempts`,
              "info",
              true,
            );
            break;
            ...
            this.log(
              `[WARNING] URL field not found after bookmark click attempt ${bookmarkClickAttempts}`,
              "warning",
              true,
            );
            ...
            await page.screenshot({
              path: attemptScreenshotPath,
              fullPage: true,
            });
            const attemptCloudinaryResult = await cloudinary.uploader.upload(
              attemptScreenshotPath,
            );
            fs.unlinkSync(attemptScreenshotPath);
            this.log(
              `[DEBUG] Bookmark attempt ${bookmarkClickAttempts} screenshot: ${attemptCloudinaryResult.secure_url}`,
              "info",
              true,
            );
        ...
        if (!urlInput || !foundUrlSelector) {
          ...
          fs.unlinkSync(debugScreenshotPath);
          this.log(
            `[DEBUG] Final URL debug screenshot uploaded: ${debugCloudinaryResult.secure_url}`,
            "info",
            true,
          );

          throw new Error(
            `Could not find URL input field after ${maxBookmarkClickAttempts} bookmark click attempts`,
          );
        }

        // Step 8b: Fill the URL field with human-like behavior (moved inside the same try block)
        this.log(
          "[EVENT] Filling URL field with human-like behavior...",
          "detail",
          false,
        );
        ...
        try {
          ...
          for (const selector of possibleNextSelectors) {
            try {
              ...
              this.log(
                `[EVENT] Found Next button with selector: ${selector}`,
                "info",
                true,
              );
              break;
              ...
              this.log(
                `[DEBUG] Next selector ${selector} not found, trying next...`,
                "detail",
                false,
              );
              continue;
          ...
          if (nextButton && foundNextSelector) {
            ...
            this.log(
              "[EVENT] Clicked Next button with human-like behavior - waiting for comment field to become visible",
              "info",
              true,
            );
            ...
            this.log(
              "[WARNING] Could not find Next button with any selector",
              "warning",
              true,
            );
          }
          ...
          this.log(
            `[WARNING] Error clicking Next button: ${nextError.message}`,
            "warning",
            true,
          );
        }
        ...
        throw new Error(
          `Could not find or click Bookmark link: ${bookmarkLinkError.message}`,
        );
      }

      // Step 8c: Clear auto-filled title and fill our own title with human-like behavior
      this.log(
        "[EVENT] Clearing auto-filled title and filling our own with human-like behavior...",
        "detail",
        false,
      );
      try {
        const titleInput = page.locator(
          'input#bookmark_item_title[name="title"]',
        );
        ...
        this.log(
          `[WARNING] Could not fill title field: ${titleError.message}`,
          "warning",
          true,
        );
      ...
      try {
        ...
        for (const selector of possibleCommentSelectors) {
          try {
            ...
            this.log(
              `[EVENT] Found comment textarea with selector: ${selector}`,
              "info",
              true,
            );
            break;
            ...
            this.log(
              `[DEBUG] Comment selector ${selector} not found, trying next...`,
              "detail",
              false,
            );
            continue;
        ...
        if (!commentTextarea || !foundCommentSelector) {
          ...
          await page.screenshot({
            path: debugCommentScreenshotPath,
            fullPage: true,
          });
          const debugCommentCloudinaryResult = await cloudinary.uploader.upload(
            debugCommentScreenshotPath,
          );
          fs.unlinkSync(debugCommentScreenshotPath);
          this.log(
            `[DEBUG] Comment debug screenshot uploaded: ${debugCommentCloudinaryResult.secure_url}`,
            "info",
            true,
          );

          throw new Error(
            "Could not find comment textarea with any of the attempted selectors",
          );
        }
        ...
        this.log(
          `[EVENT] Filled content: ${contentToPost.substring(0, 100)}...`,
          "info",
          true,
        );
      } catch (commentError) {
        throw new Error(
          `Could not fill comment field: ${commentError.message}`,
        );
      }
      ...
      try {
        ...
        if (userInfo.target_keywords) {
          const keywords = userInfo.target_keywords
            .split(",")
            .map((k) => k.trim());
          tags.push(...keywords);
        }
      ...
      this.log(
        "[EVENT] Submitting the bookmark post with human-like behavior...",
        "detail",
        false,
      );
      try {
        const postButton = page.locator(
          'input.firstIBtn[type="submit"][value="Post"]',
        );
        await postButton.waitFor({ state: "visible", timeout: 10000 });

        this.log(
          "[EVENT] Found Post button - submitting with human-like behavior...",
          "info",
          true,
        );
        ...
        this.log(
          "[EVENT] Bookmark post submitted successfully!",
          "success",
          true,
        );
      } catch (postError) {
      ...
      this.log(
        "[EVENT] Taking screenshot before permalink extraction...",
        "detail",
        false,
      );
      const beforePermalinkScreenshotPath = `screenshot_before_permalink_${this.requestId}.png`;
      await page.screenshot({
        path: beforePermalinkScreenshotPath,
        fullPage: true,
      });
      const beforePermalinkCloudinaryResult = await cloudinary.uploader.upload(
        beforePermalinkScreenshotPath,
      );
      fs.unlinkSync(beforePermalinkScreenshotPath);
      this.log(
        `[DEBUG] Before permalink screenshot: ${beforePermalinkCloudinaryResult.secure_url}`,
        "info",
        true,
      );
      ...
      this.log(
        "[EVENT] Getting permalink of the newly posted bookmark...",
        "detail",
        false,
      );
      let permalink = null;
      ...
      try {
        ...
        this.log(
          '[EVENT] Looking for the first "more" button to get permalink...',
          "info",
          true,
        );
        ...
        this.log(
          '[EVENT] Found "more" button - clicking to open dropdown...',
          "info",
          true,
        );
        ...
        const linkToItemButton = page
          .locator('a:has-text("Link to this item")')
          .first();
        await linkToItemButton.waitFor({ state: "visible", timeout: 5000 });

        this.log(
          '[EVENT] Found "Link to this item" - clicking to navigate...',
          "info",
          true,
        );
        ...
        permalink = page.url();
        this.log(
          `[EVENT] Successfully navigated to bookmark page: ${permalink}`,
          "success",
          true,
        );

        this.log(
          `[EVENT] Found permalink of newly posted bookmark: ${permalink}`,
          "success",
          true,
        );
      } catch (permalinkError) {
        this.log(
          `[WARNING] Could not get permalink: ${permalinkError.message}`,
          "warning",
          true,
        );
        ...
        try {
          this.log(
            "[EVENT] Trying alternative permalink extraction method...",
            "detail",
            false,
          );
          ...
          const urlToPost =
            ...
            userInfo.public_website_1 ||
            "https://example.com";
          const bookmarkWithOurUrl = page
            .locator(
              `a[href*="${urlToPost.replace("https://", "").replace("http://", "")}"]`,
            )
            .first();

          if (await bookmarkWithOurUrl.isVisible({ timeout: 5000 })) {
            // Get the parent container and look for more button
            const parentContainer = bookmarkWithOurUrl
              .locator(
                'xpath=ancestor::*[contains(@class, "item") or contains(@class, "bookmark") or contains(@class, "post")]',
              )
              .first();
            const moreButtonInParent = parentContainer
              .locator('a.menuItem:has-text("more"), a:has-text("more")')
              .first();
            ...
            if (await moreButtonInParent.isVisible({ timeout: 3000 })) {
              ...
              this.log(
                `[EVENT] Found permalink using alternative method: ${permalink}`,
                "success",
                true,
              );
            }
          ...
          this.log(
            `[DEBUG] Alternative permalink method also failed: ${alternativeError.message}`,
            "detail",
            false,
          );
        }
        ...
        await page.screenshot({
          path: permalinkDebugScreenshotPath,
          fullPage: true,
        });
        const permalinkDebugCloudinaryResult = await cloudinary.uploader.upload(
          permalinkDebugScreenshotPath,
        );
        fs.unlinkSync(permalinkDebugScreenshotPath);
        this.log(
          `[DEBUG] Permalink debug screenshot: ${permalinkDebugCloudinaryResult.secure_url}`,
          "info",
          true,
        );
      ...
      return {
        ...
        message:
          "Diigo Forums bookmark post created successfully with permalink!",
      };
    } catch (error) {
      ...
      if (page) {
        try {
          ...
          this.log(
            `[ERROR] Could not take error screenshot: ${screenshotError.message}`,
            "error",
            true,
          );
        }
      ...
      if (browser) {
        ...
        this.log(
          "[EVENT] Browser instance was not created or was null.",
          "warning",
          true,
        );
      }
