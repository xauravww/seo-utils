...
class IndiabookClassifiedAdapter extends BaseAdapter {
    ...
    async publish() {
        ...
        try {
            ...
            // --- Parse content JSON for title/description if possible ---
            let parsedContent = {};
            try {
            ...
            // City/State: try to get from user, else use country
            const userCity = user.city || userCountry;
            const userState = user.state || userCountry;
            // Zip: try to get from user, else random 6-digit
            ...
            // Use a simpler selector for the category dropdown
            const categorySelect = page.locator('select[name="Category"]');
            const options = await categorySelect.locator('option').all();
            let selected = false;
            for (const option of options) {
                const text = await option.textContent();
                const value = await option.getAttribute('value') || text; // Fallback to text if value is null
            ...
            if (!selected) {
                // Select the first non-placeholder option (skip '---')
                for (const option of options) {
                    const text = await option.textContent();
            ...
            await page.locator('input[name="Fax"]').fill(''); // leave blank
            ...
            // Try to submit the form using either SUBMIT or BUTTON type
            ...
            // After submission, check for errors in <ul><li><strong class="error">...</strong></li></ul>
...
class OClickerClassifiedAdapter extends BaseAdapter {
    ...
    async publish() {
        ...
        try {
            ...
            // Extra: Screenshot before clicking submit
            ...
            // Check for duplicate ad error or any alert-danger after submit
            ...
            if (await errorDiv.count() > 0 && await errorDiv.isVisible()) {
                ...
                this.log(`[ERROR] Alert after submit: ${errorText}`, 'error', true); // Always log the error message
...
// Adapter map declaration this will be kept always down to prevent the can't acess before initialization error
const adapterMap = {
